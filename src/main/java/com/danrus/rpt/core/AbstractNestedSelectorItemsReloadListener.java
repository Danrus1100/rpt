...
public abstract class AbstractNestedSelectorItemsReloadListener<T> extends SimplePreparableReloadListener<Map<ResourceLocation, List<NestedSelector<T>>>> {
    ...
    private volatile Map<ResourceLocation, List<NestedSelector<T>>> values = Map.of();
    ...
    @Override
    protected Map<ResourceLocation, List<NestedSelector<T>>> prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        Map<ResourceLocation, List<NestedSelector<T>>> map = new HashMap<>();
    ...
    private NestedSelector<T> parseResource(ResourceLocation location, Resource resource) {
    ...
    @Override
    protected void apply(Map<ResourceLocation, List<NestedSelector<T>>> object, ResourceManager resourceManager, ProfilerFiller profiler) {
        values = object;
    }

    protected abstract String getNameOfObjective();
    protected abstract ResourceLocation prepareLocation(ResourceLocation rawLocation);

    protected Optional<List<NestedSelector<T>>> getSelectors(ResourceLocation rawLocation) {
        return Optional.ofNullable(values.get(prepareLocation(rawLocation)));
    }


    public void callback(ResourceLocation rawLocation, ItemStack stack, @Nullable LivingEntity entity, T original, Consumer<T> callback) {
